 ...
 */
public class BluetoothChatHandler {
    // Debugging
     ...
     * @param mode    test mode: client or server
     */
    public BluetoothChatHandler(Context context, int mode) {
        this(context, BluetoothAdapter.getDefaultAdapter(), mode);
    }
     ...
     * @param mode    test mode. client or server
     */
    public BluetoothChatHandler(Context context, BluetoothAdapter adapter, int mode) {
        mAdapter = adapter;
        mCurrentMode = mode;
        if (mCurrentMode == MODE_SERVER) {
            mBTClientList = new ArrayList<>();
        }
    }
     ...
     *
     * @param device device  The BluetoothDevice to connectisse
     */
     ...
     *
     * @return Return the count of client which connected to server
     */
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (!isStop) {
                        BluetoothChatHandler.this.startServer();
                    }
                    break;
                }
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            synchronized (BluetoothChatHandler.this) {
                mConnectThread = null;
            }
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            int bytes;
            boolean isContinueToRead = false;
            int msgLength = -1, readLength = -1;
            ...
            while (true) {
                try {
                    ...
                    String readMsg = resultStream.toString("UTF-8");
                    Log.i("T", "readMsg=" + readMsg);
                    if (!isContinueToRead && readMsg.startsWith("M")) {
                        msgLength = Integer.parseInt(readMsg.substring(1, readMsg.indexOf(":")));
                        int colonIndex = 0;
                        for (byte tempByte : buffer) {
                            if (tempByte == ':') {
                                break;
                            }
                            colonIndex++;
                        }
                        readLength = bytes - (colonIndex + 1);
                        if (msgLength > readLength) {
                            isContinueToRead = true;
                        }
                    } else {
                        readLength += bytes;
                        Log.i("T", "readLength = " + readLength);
                        if (readLength >= msgLength) {
                            isContinueToRead = false;
                        }
                    }

                    if (!isContinueToRead) {
                        String messageText = resultStream.toString("UTF-8");
                        Message msg = mHandler.obtainMessage();
                        msg.what = MSG_READ_DATA;
                        msg.obj = mRemoteDeviceAddress + "" + messageText.substring(messageText.indexOf(":") + 1).trim();
                        msg.sendToTarget();
    ...
    public abstract interface OnWisBluetoothChatStateChangedListener {
         ...
         *
         * @param remoteName the Bluetooth name
         */
    ...
    public abstract interface OnWisBluetoothChatDataChangedListener {
         ...
         *
         * @param from the source MAC address which send the message
