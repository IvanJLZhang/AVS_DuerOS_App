package com.wistron.demo.tool.teddybear.scene.play_memo;
...
public class RecordWav {
    ...
    private static RecordWav result = null;
    private static Handler mMainHandler;

    public static RecordWav getInstanse(Context context, Boolean recordingCompressed, Handler mainHandler) {
        mContext = context;
        mMainHandler = mainHandler;
        if (recordingCompressed) {
            result = new RecordWav(false, AudioSource.MIC,
                    sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 3;
            do {
                result = new RecordWav(true, AudioSource.MIC,
                        sampleRates[i], AudioFormat.CHANNEL_CONFIGURATION_MONO,
                        AudioFormat.ENCODING_PCM_16BIT);

            } while ((++i < sampleRates.length)
                    & !(result.getState() == RecordWav.State.INITIALIZING));
        }
        return result;
    }
     ...
     */
    public RecordWav(boolean uncompressed, int audioSource,
        ...
        try {
            ...
            if (e.getMessage() != null) {
                Log.e(RecordWav.class.getName(), e.getMessage());
            } else {
                Log.e(RecordWav.class.getName(),
     ...
     */
    public void start() {
        /*HotwordResumeHelper hotwordResumeHelper = HotwordResumeHelper.getInstance(null);
        hotwordResumeHelper.stopListening();*/
        ...
        if (state == State.READY) {
            ...
            LedForRecording.recordingStart(null);
        } else {
            Log.e(RecordWav.class.getName(),
    ...
    public void stop() {
        if (state == State.RECORDING) {
            ...
            state = State.STOPPED;
            LedForRecording.recordingStop(mContext);
        } else {
            Log.e(RecordWav.class.getName(),
        ...
        /*HotwordResumeHelper hotwordResumeHelper = HotwordResumeHelper.getInstance(null);
        hotwordResumeHelper.startListening();*/
//        mMainHandler.sendEmptyMessage(MSG_START_SVA_SERVICE);
    ...
    public double calculate_dB() {
        long v = 0;
        for (int c = 0; c < readsize; c++) {
            v += buffer[c] * buffer[c];
        }
        // berlin change 5-24-2016 : readsize  ->>  audiodata.length
        double mean = v / (double) readsize;
        double volume = 10 * Math.log10(mean);
        Log.v("berlin...:", "分贝" + volume);
        try {
            Thread.sleep(200);
//                this.wait(1000);
